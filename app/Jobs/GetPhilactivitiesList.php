...
class GetPhilactivitiesList implements ShouldQueue
{
    ...
    public function handle()
    {
		...
		$request = 'https://api.tickets.yandex.net/api/agent/?action=activity.list&offset=0&limit=500&city_id='.$cid_phila.'&auth='.$auth;
		...
		if(intval($json['status']) === 0) {
			if(array_key_exists('result', $json)) {
				...
				if(!Philactivity::upsert(
					...
					Log::channel('teatrapich')->info('Activities for Phila NOT saved!');
				}
			}
		}
