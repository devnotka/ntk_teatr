									...
									@isset($event->activity->actual_seances)
										@if(count($event->activity->actual_seances) > 0)
												...
												@isset($event->extraFields->premiere)
													@if($event->extraFields->premiere === true)
														<a href="{{ route('afisha.variant', [ 'variant' => 'premiers' ]) }}" alt="Премьеры в Новгородском театре драмы">
															<div class="mr-1 px-2 py-[15px] rounded font-bold text-sm text-center text-white uppercase bg-pink-700">
																Премьера!
															</div>
														</a>
													@endif
											...
											@isset($event->statusPushka)
												<div class="flex flex-row rounded border border-gray-300">
													<div class="text-sm text-center text-black uppercase transition-all duration-300 ease-in-out rounded-l shadow hover:shadow-lg">
														<x-teatr.pushkin_icon></x-teatr.pushkin_icon>
													</div>
													<div class="mr-1 px-2 py-1 text-sm transition-all duration-300 ease-in-out">
														Доступно<br /> по Пушкинской карте
													</div>
												</div>
											@endisset
										@endif
						...
						<div id="seances" class="px-4 pb-4 xl:pb-8">
							...
							<div class="flex flex-col pt-2">
								@foreach($actual_seances as $actual_seance)
										@if($actual_seance->datechange)
											<div class="relative flex flex-row px-2 pb-4 py-1 md:px-4 md:pb-5 md:py-2 mt-2 -mb-4 rounded text-sm md:text-base bg-gray-200 dark:bg-gray-600">
												Внимание! Событие перенесено с {{ $actual_seance->getHumanDate($actual_seance->datechange) }} на {{ $actual_seance->getHumanDate($actual_seance->date) }}. Все ранее приобретенные билеты действительны на новую дату!
											</div>
										@endif
										<div class="relative flex flex-row px-2 py-1 md:px-4 md:py-2 my-2 rounded text-sm md:text-base bg-gray-100 dark:bg-gray-700">
											<div class="w-44">
												{!! $actual_seance->getHumanDateForEventRow($actual_seance->date) !!}
											</div>
											<div class="relative flex-grow">
												@if($actual_seance->replace)
													<a href="{{ route('repertuar.id', [ 'id' => $actual_seance->getEventBySeanceId($actual_seance->replace)->activity->event->id ]) }}">
												...
												@elseif($actual_seance->is_cancel === 1)
													<div class="absolute right-0 top-1/2 -mt-5 block bg-pink-700 font-bold text-sm text-center text-white rounded">
														<span class="inline-block px-4 pt-3 pb-1 uppercase overflow-hidden">
															Отменено!
														</span>
														...
														@if($actual_seance->min_price != 0)
															<span class="inline-block px-4 overflow-hidden" style="margin-bottom:-7px;">
																<div class="w-16 whitespace-nowrap transition duration-300 ease-in-out transform -translate-x-24 hover:translate-x-0">
																	...
																	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
																	<span class="">от&nbsp;{{ $actual_seance->min_price }}₽</span>
																</div>
															</span>
														@else
															<span class="inline-block px-4 overflow-hidden" style="margin-bottom:-7px;">
																<div class="w-16 whitespace-nowrap transition duration-300 ease-in-out translate-x-0">
