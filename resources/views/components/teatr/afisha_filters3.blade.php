<div class="sticky z-1 top-4">
	<div x-data="{ filters_open: false }" :class="{'pb-6': filters_open, 'pb-2': !filters_open}" class="md:pb-0 md:pr-8">
		...
		<h2 class="hidden uppercase mb-2 md:block">
			...
			Фильтры
			<a class="absolute top-2 right-8 text-xs lowercase transition duration-300 ease-in-out hover:text-tegold" href="{{ route('afisha') }}">
				<svg fill="currentColor" viewBox="0 0 20 20" class="inline-block w-3 h-3">
					<path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path>
				</svg>
				Сбросить
		...
		<div :class="{'block': filters_open, 'hidden': !filters_open}" class="transition-all duration-300 ease-in-out md:block">
				...
				<div class="block w-100 px-2 py-2 mt-4 md:mt-8 mb-4 md:mb-8 rounded border-2 border-dashed border-gray-300 dark:border-gray-700 dark:text-gray-100 backdrop-blur-sm">
					...
					<nav class="flex flex-wrap pt-1 pl-1 items-center text-sm text-white justify-start">
						@foreach($variants as $variant)
							<a class="{{ Request::is('afisha/'.$variant['sysName']) ? 'bg-tegold hover:bg-yellow-600' : 'bg-slate-600 hover:bg-slate-700' }} transition duration-300 ease-in-out px-2 py-1 mr-1 mb-1 rounded" href="{{ route('afisha.variant', [ 'variant' => $variant['sysName'] ]) }}">
								{{ $variant['name'] }}
							</a>
						@endforeach
					</nav>
				...
				<div class="block w-100 px-2 py-2 mb-4 md:mb-8 rounded border-2 border-dashed border-gray-300 dark:border-gray-700 dark:text-gray-100 backdrop-blur-sm">
					...
					<nav class="flex flex-wrap pt-1 pl-1 items-center text-sm text-white justify-start">
						@foreach($categories as $category)
							<a class="{{ Request::is('afisha/category/'.$category['sysName']) ? 'bg-tegold hover:bg-yellow-600' : 'bg-slate-600 hover:bg-slate-700' }} transition duration-300 ease-in-out px-2 py-1 mr-1 mb-1 rounded" href="{{ route('afisha.category', [ 'category' => $category['sysName'] ]) }}">
								{{ $category['name'] }}
							</a>
						@endforeach
					</nav>
			...
			@isset($tags)
				<div class="block w-100 px-2 py-2 mb-4 md:mb-8 rounded border-2 border-dashed border-gray-300 dark:border-gray-700 dark:text-gray-100 backdrop-blur-sm">
					...
					<nav class="flex flex-wrap pt-1 pl-1 items-center text-sm text-white justify-start">
						@foreach($tags as $tag)
							<a class="{{ Request::is('afisha/tag/'.$tag['sysName']) ? 'bg-tegold hover:bg-yellow-600' : 'bg-slate-600 hover:bg-slate-700' }} lowercase transition duration-300 ease-in-out px-2 py-1 mr-1 mb-1 rounded" href="{{ route('afisha.tag', [ 'tag' => $tag['sysName'] ]) }}">
								{{ $tag['name'] }}
							</a>
						@endforeach
					</nav>
			...
			@endisset
			@isset($prices)
				<div class="block w-100 px-2 py-2 rounded border-2 border-dashed border-gray-300 dark:border-gray-700 dark:text-gray-100 backdrop-blur-sm">
					<span class="absolute -mt-6 px-2 bg-white dark:bg-gray-800">
						<svg xmlns="http://www.w3.org/2000/svg" class="inline-block h-4 w-4 -mt-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
					...
					<nav class="flex flex-wrap pt-1 pl-1 items-center text-sm text-white justify-start">
						@foreach($prices as $price)
							<a class="{{ Request::is('afisha/price/'.$price['sysName']) ? 'bg-tegold hover:bg-yellow-600' : 'bg-slate-600 hover:bg-slate-700' }} transition duration-300 ease-in-out px-2 py-1 mr-1 mb-1 rounded" href="{{ route('afisha.price', [ 'price' => $price['sysName'] ]) }}">
								{{ $price['name'] }}
							</a>
						@endforeach
					</nav>
